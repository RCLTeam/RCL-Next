...
import { boolean, check, date, foreignKey, index, integer, jsonb, pgEnum, pgTable,
  primaryKey, smallint, text, timestamp, unique, uniqueIndex, uuid, varchar } from 'drizzle-orm/pg-core';
...
export const auditLogs = pgTable('audit_logs', {
  ...
  foreignKey({ name: 'audit_logs_actor_discord_user_id_fkey', columns: [t.actorDiscordUserId], foreignColumns: [discordUsers.discordId] }).onDelete('set null'),
...
export const seasons = pgTable('seasons', {
  ...
  check('seasons_dates_check', sql`"ends_on" IS NULL OR "starts_on" IS NULL OR "ends_on" >= "starts_on"`),
...
export const seasonsDivisions = pgTable('seasons_divisions', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  foreignKey({ name: 'seasons_divisions_season_name_fkey', columns: [t.seasonName], foreignColumns: [seasons.name] }).onDelete('cascade'),
  foreignKey({ name: 'seasons_divisions_division_name_fkey', columns: [t.divisionName], foreignColumns: [divisions.name] }).onDelete('cascade'),
...
export const players = pgTable('players', {
  ...
  foreignKey({ name: 'players_discord_user_id_fkey', columns: [t.discordUserId], foreignColumns: [discordUsers.discordId] }).onDelete('set null'),
...
export const teams = pgTable('teams', {
  ...
  foreignKey({ name: 'teams_season_division_id_fkey', columns: [t.seasonDivisionId], foreignColumns: [seasonsDivisions.id] }).onDelete('cascade'),
...
export const teamMemberships = pgTable('team_memberships', {
  ...
  foreignKey({ name: 'team_memberships_team_id_fkey', columns: [t.teamId], foreignColumns: [teams.id] }).onDelete('cascade'),
  foreignKey({ name: 'team_memberships_discord_user_id_fkey', columns: [t.discordUserId], foreignColumns: [discordUsers.discordId] }).onDelete('cascade'),
  ...
  check('team_memberships_captain_role_check', sql`"is_captain" = false OR "role" IN ('top', 'jungle', 'mid', 'adc', 'support')`),
  index('team_memberships_team_id_idx').on(t.teamId),
  index('team_memberships_discord_user_id_idx').on(t.discordUserId),
  uniqueIndex('team_memberships_unique_captain').on(t.teamId).where(sql`"team_memberships"."is_captain" = true`)
]);
...
export const rounds = pgTable('rounds', {
  ...
  foreignKey({ name: 'rounds_id_season_division_fkey', columns: [t.idSeasonDivision], foreignColumns: [seasonsDivisions.id] }).onDelete('cascade'),
...
export const matches = pgTable('matches', {
  ...
  foreignKey({ name: 'matches_id_season_division_fkey', columns: [t.idSeasonDivision], foreignColumns: [seasonsDivisions.id] }).onDelete('cascade'),
  foreignKey({ name: 'matches_team1_id_fkey', columns: [t.team1Id], foreignColumns: [teams.id] }).onDelete('cascade'),
  foreignKey({ name: 'matches_team2_id_fkey', columns: [t.team2Id], foreignColumns: [teams.id] }).onDelete('cascade'),
  foreignKey({ name: 'matches_winner_team_id_fkey', columns: [t.winnerTeamId], foreignColumns: [teams.id] }).onDelete('set null'),
  ...
  check('matches_winner_participant_check', sql`"winner_team_id" IS NULL OR "winner_team_id" IN ("team1_id", "team2_id")`),
  foreignKey({ name: 'matches_round_fkey', columns: [t.idRound, t.idSeasonDivision], foreignColumns: [rounds.id, rounds.idSeasonDivision] }).onDelete('set null'),
...
export const matchGames = pgTable('match_games', {
  ...
  foreignKey({ name: 'match_games_matches_id_fkey', columns: [t.matchesId], foreignColumns: [matches.id] }).onDelete('cascade'),
  foreignKey({ name: 'match_games_blue_team_id_fkey', columns: [t.blueTeamId], foreignColumns: [teams.id] }).onDelete('cascade'),
  foreignKey({ name: 'match_games_red_team_id_fkey', columns: [t.redTeamId], foreignColumns: [teams.id] }).onDelete('cascade'),
  foreignKey({ name: 'match_games_winner_team_id_fkey', columns: [t.winnerTeamId], foreignColumns: [teams.id] }).onDelete('cascade'),
  ...
  check('match_games_winner_participant_check', sql`"winner_team_id" IS NULL OR "winner_team_id" IN ("blue_team_id", "red_team_id")`),
...
export const playerGameInfo = pgTable('player_game_info', {
  ...
  foreignKey({ name: 'player_game_info_match_game_id_fkey', columns: [t.matchGameId], foreignColumns: [matchGames.id] }).onDelete('cascade'),
  foreignKey({ name: 'player_game_info_player_id_fkey', columns: [t.playerId], foreignColumns: [players.id] }).onDelete('restrict'),
  foreignKey({ name: 'player_game_info_team_id_fkey', columns: [t.teamId], foreignColumns: [teams.id] }).onDelete('restrict'),
...
export const playerGameBuild = pgTable('player_game_build', {
  ...
  foreignKey({ name: 'player_game_build_id_fkey', columns: [t.id], foreignColumns: [playerGameInfo.id] }).onDelete('cascade')
]);
...
export const playerGameRunes = pgTable('player_game_runes', {
  ...
  foreignKey({ name: 'player_game_runes_id_fkey', columns: [t.id], foreignColumns: [playerGameInfo.id] }).onDelete('cascade')
]);
...
export const playerGameStats = pgTable('player_game_stats', {
  ...
  foreignKey({ name: 'player_game_stats_id_fkey', columns: [t.id], foreignColumns: [playerGameInfo.id] }).onDelete('cascade'),
  check('player_game_stats_non_negative_check', sql`"kills" >= 0 AND "deaths" >= 0 AND "assists" >= 0 AND "cs" >= 0 AND "damage_to_champions" >= 0`),
...
export const predictions = pgTable('predictions', {
  ...
  foreignKey({ name: 'predictions_discord_user_id_fkey', columns: [t.discordUserId], foreignColumns: [discordUsers.discordId] }).onDelete('cascade'),
  foreignKey({ name: 'predictions_match_id_fkey', columns: [t.matchId], foreignColumns: [matches.id] }).onDelete('cascade'),
  foreignKey({ name: 'predictions_selected_team_id_fkey', columns: [t.selectedTeamId], foreignColumns: [teams.id] }).onDelete('restrict'),
