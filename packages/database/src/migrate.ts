...
try {
  ...
  try {
    ...
    if (rows[0]?.exists && !tracking.rows[0]?.exists) {
      throw new Error('Existing untracked schema detected. Use a new empty database; do not baseline production automatically.');
    }
    if (tracking.rows[0]?.exists) {
      ...
      if (applied.rows.some((row, index) => row.hash !== files[index]?.hash)) {
        throw new Error('Migration history differs from these files. Plan an explicit upgrade; existing data was not modified.');
      }
    }
