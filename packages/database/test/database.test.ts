...
test('PostgreSQL migrations, fixtures and relational constraints', async t => {
  ...
  await t.test('SQL columns, foreign keys, checks and indexes match Drizzle metadata', async () => {
    for (const table of Object.values(schema).filter(value => is(value, Table))) {
      ...
      const columns = await client.query<{ name: string; not_null: boolean }>(`
        ...
        WHERE attrelid=$1::regclass AND attnum>0 AND NOT attisdropped ORDER BY attname`, [config.name]);
      assert.deepEqual(columns.rows, config.columns.map(column => ({
        name: column.name, not_null: column.notNull
      })).sort((a,b) => a.name.localeCompare(b.name, 'en')), config.name + ' columns');
      const keys = await client.query<{ name: string }>(`
        SELECT conname AS name FROM pg_constraint WHERE conrelid=$1::regclass AND contype='f' ORDER BY conname`, [config.name]);
      assert.deepEqual(keys.rows.map(row => row.name).sort(), config.foreignKeys.map(key => key.getName()).sort(), config.name + ' foreign keys');
      const checks = await client.query<{ name: string }>(`
        SELECT conname AS name FROM pg_constraint WHERE conrelid=$1::regclass AND contype='c'`, [config.name]);
      assert.deepEqual(checks.rows.map(row => row.name).sort(), config.checks.map(check => check.name).sort(), config.name + ' checks');
      const indexes = await client.query<{ name: string }>(`
        ...
        AND NOT EXISTS (SELECT 1 FROM pg_constraint c WHERE c.conindid=('public.' || i.indexname)::regclass
          AND c.contype IN ('p','u'))`, [config.name]);
      assert.deepEqual(indexes.rows.map(row => row.name).sort(), config.indexes.map(index => index.config.name).sort(), config.name + ' indexes');
    }
  });
  ...
  await t.test('Discord identities own rosters and may have multiple game accounts', async () => {
    ...
    await client.query('DELETE FROM players WHERE id=ANY($1::uuid[])', [accounts.rows.map(row => row.id)]);
  });
  await t.test('captain and composite roster keys are enforced', async () => {
    await assert.rejects(client.exec(`UPDATE team_memberships SET is_captain=true
      WHERE team_id='30000000-0000-4000-8000-000000000001' AND discord_user_id='900000000000000002'`));
    await assert.rejects(client.exec(`UPDATE team_memberships SET role='coach'
      WHERE team_id='30000000-0000-4000-8000-000000000001' AND is_captain`));
    await assert.rejects(client.exec(`INSERT INTO team_memberships (team_id, discord_user_id, role)
      SELECT team_id, discord_user_id, role FROM team_memberships LIMIT 1`));
  });
  await t.test('round numbers are scoped to season/division and predictions use Discord IDs', async () => {
    const result = await client.query<{ total: number }>('SELECT count(*)::int AS total FROM rounds WHERE id=1');
