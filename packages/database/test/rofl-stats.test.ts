...
const extraStats = [
  ...
  [
    ...
    "daño_recibido_campeones"
  ],
  [
    "damageMitigated",
    "soporte.daño_mitigado"
  ],
  [
    "crowdControlTime",
    "soporte.control_adversarios"
  ],
  [
    "turretsKilled",
    "estructuras.torres"
  ...
  [
    "turretTakedowns",
    "estructuras.derribos_torres"
  ],
  [
    "inhibitorsKilled",
    "estructuras.inhibidores"
  ],
  [
    "inhibitorTakedowns",
    "estructuras.derribos_inhibidores"
  ],
  [
    "wardsPlaced",
    "vision.wards_colocados"
  ],
  [
    "wardsDestroyed",
    "vision.wards_destruidos"
  ],
  [
    "controlWardsPurchased",
    "vision.pinkwards_comprados"
  ],
  [
    "detectorWardsPlaced",
    "vision.wards_detector"
  ],
  [
    "pings",
    "pings"
  ...
  [
    "voidGrubsKilled",
    "monstruos.void_grubs"
  ],
  [
    "elderDragonsKilled",
    "monstruos.elder_dragons"
  ],
  [
    "objectivesStolen",
    "monstruos.objetivos_robados"
  ],
  [
    "objectivesStolenAssists",
    "monstruos.asistencias_robo"
  ],
  [
    "largestAbilityDamage",
    "gameplay.mayor_daño_habilidad"
  ],
  [
    "largestAttackDamage",
    "gameplay.mayor_daño_ataque"
  ],
  [
    "largestCriticalStrike",
    "gameplay.mayor_critico"
  ],
  [
    "longestTimeLiving",
    "gameplay.tiempo_vivo_mas_largo"
  ],
  [
    "timeSpentDead",
    "gameplay.tiempo_muerto"
  ]
] as const;
function field(source: unknown, path: string): unknown {
  return path.split('.').reduce<unknown>((value, key) =>
    typeof value === 'object' && value !== null ? Reflect.get(value, key) : undefined, source);
}
...
test('all ten ROFL JSON participants fit the normalized schema without losing metrics', async t => {
  ...
  for (let index = 0; index < participants.length; index++) {
    ...
    const stats = {
      ...Object.fromEntries(extraStats.map(([column, path]) => [column, number(participant, path)])),
    ...
    const slotMap = new Map(slots.map((slot: unknown) => [number(slot, 'slot'), number(slot, 'id')]));
    assert.equal(slotMap.size, 7);
    const build = {
      ...Object.fromEntries(Array.from({ length: 6 }, (_, slot) => ['item' + slot, slotMap.get(slot)])),
    ...
    await db.transaction(async tx => {
      await tx.update(schema.players).set({
        puuid: string(participant, 'puuid'), gameName: string(participant, 'nombre'),
        riotTag: string(participant, 'tag')
      }).where(eq(schema.players.id, '40000000-0000-4000-8000-' + suffix));
      await tx.update(schema.playerGameInfo).set({
        position: string(participant, 'posicion'), champion: string(participant, 'campeon')
    ...
    for (const [table, expected] of [
      [schema.playerGameStats, stats], [schema.playerGameRunes, runes], [schema.playerGameBuild, build]
    ] as const) {
      const [stored] = await db.select().from(table).where(eq(table.id, id));
      assert.ok(stored);
      const values = new Map(Object.entries(stored));
      for (const [key, value] of Object.entries(expected)) assert.equal(values.get(key), value, key);
    }
    const [player] = await db.select().from(schema.players).where(eq(schema.players.id, '40000000-0000-4000-8000-' + suffix));
    assert.equal(player?.puuid, string(participant, 'puuid'));
    const [info] = await db.select().from(schema.playerGameInfo).where(eq(schema.playerGameInfo.id, id));
  ...
  await t.test('absent metrics stay NULL while explicit zero is retained', async () => {
    ...
    await db.update(schema.playerGameStats).set({ goldEarned: null, pentaKills: 0 }).where(eq(schema.playerGameStats.id, id));
    const [row] = await db.select().from(schema.playerGameStats).where(eq(schema.playerGameStats.id, id));
