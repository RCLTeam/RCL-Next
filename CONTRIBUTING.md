...
- Mientras la base no esté en producción, los cambios se consolidan en `0000_initial_schema.sql`, su snapshot y una única entrada del journal. No se añaden migraciones incrementales. Si una base local ya existe, conserva sus datos y reconcilia su historial explícitamente después de comprobar que el esquema coincide; el migrador no acepta cambios de historial automáticamente. En producción se usarán migraciones incrementales.
...
- La home usa `site/pages/home/home.css` como índice de imports: `home-hero.css` (cabecera y ticker), `featured-match.css`, `weekly-team.css`, `editorial-grid.css`, `competition-teaser.css` y `crown-teaser.css`. Cada archivo contiene los estados y breakpoints de su sección. El artículo y su modal mantienen sus estilos en `features/home-content/components/editorial.css`; `editorial-tag.css` comparte la etiqueta de categoría con las tarjetas de la home.
- Para cambiar el indicador de página seleccionada, edita `site/layout/site-navigation.css`: contiene `[aria-current='page']`, hover y onda de escritorio. `SiteNavigation`, dentro de `site/layout/SiteLayout.tsx`, asigna el atributo. Las rutas, títulos, pantallas y consultas de competición necesarias se definen en `site/routes.tsx`; `shared/navigation.ts` mantiene el contexto de navegación.
