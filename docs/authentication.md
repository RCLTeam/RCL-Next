...
`0001_discord_auth.sql` añade `auth_sessions` y `oauth_states`, con índices de caducidad y borrado de sesiones en cascada al eliminar el usuario. Se conserva intacto el baseline `0000_initial_schema.sql`. El snapshot inicial contenía una referencia obsoleta a `pickem_question_type`, que no existe en ese SQL; el nuevo snapshot corrige esa discrepancia sin ejecutar un DROP sobre un tipo inexistente.

Las pruebas HTTP ejecutan ambas migraciones en PGlite y simulan únicamente las respuestas externas de Discord. Verifican cookies, intercambio de código, protección de estado, consumo concurrente, sesiones, roles, logout y errores sin filtración de secretos. Para comprobar el consentimiento real en Discord se necesitan las credenciales y una base PostgreSQL configurada.
