...
* **Separación de Identidades:** Usuarios Discord (`discord_users`) y jugadores (`players`) son entidades distintas: un espectador puede emitir pronósticos sin ser jugador. Sin embargo, la participación competitiva es estrictamente por equipos: `team_memberships` aplica restricción `discord_user_id` `NOT NULL` (vinculando la identidad física). Por consiguiente, todo invocador que participe activamente debe estar obligatoriamente vinculado a una cuenta de Discord registrada en el roster del equipo.
