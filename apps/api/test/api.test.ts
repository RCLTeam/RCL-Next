...
import type { CompetitionRepository, Match, Team } from '../src/modules/competition/competition.repository.js';
...
const teams: Team[] = [homeId, awayId].map((id, index) => ({
  id, divisionId, name: index ? 'B' : 'A', shortName: null, logoUrl: null, color: null, isActive: true
}));
const match: Match = {
  id: '70000000-0000-4000-8000-000000000001', divisionId, roundId, homeTeamId: homeId,
  awayTeamId: awayId, homeScore: 1, awayScore: 0, winnerTeamId: homeId, status: 'completed',
  bestOf: 1, scheduledAt: null, finishedAt: null, streamUrl: null
};
function repository(): CompetitionRepository {
  ...
  return {
    seasons: async () => [season], season: async id => id === seasonId ? season : undefined,
    divisions: async () => [division], division: async id => id === divisionId ? division : undefined,
    teams: async () => teams,
    rounds: async () => [{
      id: roundId, divisionId, sequence: 1, stage: 'regular', name: 'J1', startsAt: null, lockAt: null
    }, {
      id: awayId, divisionId, sequence: 1, stage: 'playoffs', name: 'Final', startsAt: null, lockAt: null
    }],
    matches: async () => [match, { ...match, roundId: awayId, winnerTeamId: awayId, homeScore: 0, awayScore: 3 },
      { ...match, id: awayId, status: 'scheduled', winnerTeamId: null, homeScore: 0, awayScore: 0 }]
  };
}
const app = createApp({ repository: repository(), checkDatabase: async () => {}, corsOrigin: 'http://localhost:5173' });

test('readiness reports database outages as 503', async () => {
  const offline = createApp({
    repository: repository(), corsOrigin: 'http://localhost:5173',
    checkDatabase: async () => { throw new Error('secret connection'); }
  });
...
test('malformed JSON and unexpected failures do not leak internals', async () => {
  ...
  const failing = createApp({
    repository: { ...repository(), seasons: async () => { throw new Error('password=secret'); } },
    corsOrigin: 'http://localhost:5173', checkDatabase: async () => {}
  });
...
test('calendar filters by round and expands teams', async () => {
  const result = await request(app).get('/api/v1/divisions/' + divisionId + '/calendar?roundId=' + roundId).expect(200);
