...
test('HTTP -> controller -> service -> real repository -> embedded PostgreSQL', async t => {
  ...
  const seed = await readFile(new URL('../../../packages/database/seed/demo.sql', import.meta.url), 'utf8');
  await client.transaction(tx => tx.exec(seed));
  const app = createApp({
    repository: new PostgresCompetitionRepository(db),
    checkDatabase: async () => { await db.select().from(schema.seasons).limit(1); },
  ...
  const ascend = await request(app).get('/api/v1/divisions/' + divisions.body.data[1].id + '/standings').expect(200);
